#!/usr/bin/env bash
# Post-deploy production configuration smoke test for hosting-service.
# Run against a live HTTPS deployment (platform API host, not every app subdomain).
#
# Usage:
#   ./tools/prod-config-check.sh https://hosting.example.com
#   ./tools/prod-config-check.sh https://hosting.example.com --slug vpzzsxvzsmp7
#   ./tools/prod-config-check.sh https://hosting.example.com --app-host https://vpzzsxvzsmp7.shellui.app
#
# Optional env:
#   HOSTING_APP_DOMAIN   Default app subdomain suffix (default: shellui.app)
#   CORS_PROBE_ORIGIN    Origin for CORS preflight (default: random preview-style origin)
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

usage() {
  cat <<'EOF'
Usage: ./tools/prod-config-check.sh BASE_URL [--slug SLUG] [--app-host URL]

Verify a deployed hosting-service platform instance is correctly configured for production.

Environment (optional):
  HOSTING_APP_DOMAIN   Suffix for --slug app URLs (default: shellui.app)
  CORS_PROBE_ORIGIN    Origin header for CORS preflight probe

Checks: HTTPS reachability, protected /hosting/v1/* probes, public health + llms.txt,
        bootstrap gate on /, identity JWKS wiring (via health), CORS, optional app-host
        smoke with --slug/--app-host, security headers (warn-only).

Exit 0 when all hard checks pass; non-zero if any FAIL.
EOF
}

pass() { printf 'PASS: %s\n' "$*"; }
fail() { printf 'FAIL: %s\n' "$*" >&2; FAILURES=$((FAILURES + 1)); }
warn() { printf 'WARN: %s\n' "$*" >&2; }
info() { printf 'INFO: %s\n' "$*"; }

require_cmd() {
  command -v "$1" >/dev/null 2>&1 || {
    printf 'FAIL: required command not found: %s\n' "$1" >&2
    exit 2
  }
}

normalize_base_url() {
  local url="${1%/}"
  if [[ ! "${url}" =~ ^https?:// ]]; then
    printf 'FAIL: BASE_URL must include scheme (https://…)\n' >&2
    exit 2
  fi
  printf '%s' "${url}"
}

url_origin() {
  python3 - "$1" <<'PY'
import sys
from urllib.parse import urlsplit
parts = urlsplit(sys.argv[1])
print(f"{parts.scheme}://{parts.netloc}")
PY
}

FAILURES=0
BASE_URL=""
PREVIEW_SLUG="${PREVIEW_SLUG:-}"
APP_HOST_URL="${APP_HOST_URL:-}"
HOSTING_APP_DOMAIN="${HOSTING_APP_DOMAIN:-shellui.app}"
CORS_PROBE_ORIGIN="${CORS_PROBE_ORIGIN:-https://example-preview-slug.shellui.app}"

while [[ $# -gt 0 ]]; do
  case "$1" in
    -h|--help)
      usage
      exit 0
      ;;
    --slug)
      PREVIEW_SLUG="${2:-}"
      [[ -n "${PREVIEW_SLUG}" ]] || {
        printf 'FAIL: --slug requires a value\n' >&2
        exit 2
      }
      shift 2
      ;;
    --app-host)
      APP_HOST_URL="${2:-}"
      [[ -n "${APP_HOST_URL}" ]] || {
        printf 'FAIL: --app-host requires a value\n' >&2
        exit 2
      }
      shift 2
      ;;
    --)
      shift
      break
      ;;
    -*)
      printf 'FAIL: unknown option: %s\n' "$1" >&2
      usage >&2
      exit 2
      ;;
    *)
      if [[ -z "${BASE_URL}" ]]; then
        BASE_URL="$1"
      else
        printf 'FAIL: unexpected argument: %s\n' "$1" >&2
        exit 2
      fi
      shift
      ;;
  esac
done

[[ -n "${BASE_URL}" ]] || {
  usage >&2
  exit 2
}

require_cmd curl
require_cmd python3

BASE_URL="$(normalize_base_url "${BASE_URL}")"
ORIGIN="$(url_origin "${BASE_URL}")"

if [[ -n "${APP_HOST_URL}" ]]; then
  APP_HOST_URL="$(normalize_base_url "${APP_HOST_URL}")"
elif [[ -n "${PREVIEW_SLUG}" ]]; then
  APP_HOST_URL="https://${PREVIEW_SLUG}.${HOSTING_APP_DOMAIN}"
fi

info "Target: ${BASE_URL}"
info "App domain suffix (for --slug): ${HOSTING_APP_DOMAIN}"
if [[ -n "${APP_HOST_URL}" ]]; then
  info "App host smoke: ${APP_HOST_URL}"
else
  info "No --slug / --app-host — skipping public app-host smoke (optional)"
fi

# ---------------------------------------------------------------------------
# 1. Reachable (HTTPS, follow redirects)
# ---------------------------------------------------------------------------
reachable_meta="$(
  curl -sS -L -o /dev/null -w '%{http_code}\t%{url_effective}\t%{num_redirects}' \
    --connect-timeout 15 --max-time 30 \
    "${BASE_URL}/" 2>/dev/null || printf '000\t\t0'
)"
IFS=$'\t' read -r reach_code reach_final reach_redirects <<<"${reachable_meta}"

if [[ "${reach_code}" == "000" || -z "${reach_code}" ]]; then
  fail "Reachable: no response from ${BASE_URL}/ (connection/TLS/timeout)"
elif [[ "${reach_code}" =~ ^[45][0-9][0-9]$ ]]; then
  fail "Reachable: ${BASE_URL}/ returned HTTP ${reach_code} (final: ${reach_final:-unknown})"
else
  reach_note="HTTP ${reach_code}"
  if [[ "${reach_redirects:-0}" -gt 0 ]]; then
    reach_note="${reach_note}, ${reach_redirects} redirect(s) → ${reach_final}"
    if [[ "${BASE_URL}/" != "${reach_final}" && "${BASE_URL}" != "${reach_final}" ]]; then
      reach_note="${reach_note} (note SSL/host redirect)"
    fi
  fi
  if [[ "${reach_final}" == http://* && "${BASE_URL}" == https://* ]]; then
    warn "Reachable: final URL is HTTP after redirect (${reach_final}) — prefer HTTPS in production"
  fi
  pass "Reachable: ${reach_note}"
fi

# ---------------------------------------------------------------------------
# 2. Protected API surface (unauthenticated → 401/403, not 500)
# ---------------------------------------------------------------------------
for probe_path in '/hosting/v1/apps' '/hosting/v1/stats'; do
  probe_code="$(
    curl -sS -L -o /dev/null -w '%{http_code}' \
      --connect-timeout 15 --max-time 30 \
      "${BASE_URL}${probe_path}" 2>/dev/null || echo '000'
  )"
  if [[ "${probe_code}" == "500" ]]; then
    fail "API ${probe_path}: unauthenticated GET → HTTP 500 (misconfiguration — check JWKS/DB/logs)"
  elif [[ "${probe_code}" == "000" ]]; then
    fail "API ${probe_path}: no response"
  elif [[ "${probe_code}" =~ ^(401|403)$ ]]; then
    pass "API ${probe_path}: unauthenticated GET → HTTP ${probe_code} (auth required)"
  elif [[ "${probe_code}" =~ ^4[0-9][0-9]$ ]]; then
    pass "API ${probe_path}: unauthenticated GET → HTTP ${probe_code} (client error, not 500)"
  else
    warn "API ${probe_path}: unauthenticated GET → HTTP ${probe_code} (expected 401/403)"
  fi
done

# ---------------------------------------------------------------------------
# 3. Public health + llms.txt
# ---------------------------------------------------------------------------
health_body="$(mktemp)"
health_code="$(
  curl -sS -L -o "${health_body}" -w '%{http_code}' \
    --connect-timeout 15 --max-time 30 \
    "${BASE_URL}/hosting/v1/health" 2>/dev/null || echo '000'
)"

health_result="$(
  HEALTH_FILE="${health_body}" HEALTH_CODE="${health_code}" python3 <<'PY'
import json
import os
import sys

code = os.environ["HEALTH_CODE"]
path = os.environ["HEALTH_FILE"]

if code == "000":
    print("FAIL\tno response from /hosting/v1/health")
    sys.exit(0)
if code == "500":
    print("FAIL\tHTTP 500 — check database, JWKS, and container logs")
    sys.exit(0)
if code != "200":
    print(f"FAIL\tHTTP {code} (expected 200)")
    sys.exit(0)

try:
    with open(path, encoding="utf-8") as fh:
        doc = json.load(fh)
except (OSError, json.JSONDecodeError) as exc:
    print(f"FAIL\tHTTP 200 but invalid JSON: {exc}")
    sys.exit(0)

if doc.get("status") != "ok":
    print(f"FAIL\tstatus={doc.get('status')!r} (expected 'ok')")
    sys.exit(0)

jwks_source = doc.get("identity_jwks_source") or "url"
jwks_url = doc.get("identity_jwks_url") or "?"
backend = doc.get("hosting_backend") or "?"
version = doc.get("version") or "?"
print(
    f"PASS\tstatus=ok version={version} backend={backend} "
    f"identity_jwks_source={jwks_source} identity_jwks_url={jwks_url}"
)
PY
)" || health_result=$'FAIL\thealth parse error'

health_status="${health_result%%$'\t'*}"
health_detail="${health_result#*$'\t'}"
case "${health_status}" in
  PASS) pass "Health: ${health_detail}" ;;
  *) fail "Health: ${health_detail}" ;;
esac
rm -f "${health_body}"

llms_body="$(mktemp)"
llms_code="$(
  curl -sS -L -o "${llms_body}" -w '%{http_code}' \
    --connect-timeout 15 --max-time 30 \
    "${BASE_URL}/llms.txt" 2>/dev/null || echo '000'
)"

if [[ "${llms_code}" == "404" ]]; then
  info "llms.txt: not present (HTTP 404) — optional on custom installs"
elif [[ "${llms_code}" == "000" ]]; then
  fail "llms.txt: no response from GET /llms.txt"
elif [[ "${llms_code}" == "500" ]]; then
  fail "llms.txt: GET /llms.txt → HTTP 500"
elif [[ "${llms_code}" == "200" ]]; then
  if grep -qi 'shellui' "${llms_body}" 2>/dev/null; then
    pass "llms.txt: HTTP 200 with Shellui overview text"
  else
    warn "llms.txt: HTTP 200 but content looks unexpected — inspect manually"
  fi
else
  warn "llms.txt: GET /llms.txt → HTTP ${llms_code} (expected 200 or 404)"
fi
rm -f "${llms_body}"

# ---------------------------------------------------------------------------
# 4. Bootstrap gate (no open superuser form on platform apex)
# ---------------------------------------------------------------------------
home_body="$(mktemp)"
home_code="$(
  curl -sS -L -o "${home_body}" -w '%{http_code}' \
    --connect-timeout 15 --max-time 30 \
    "${BASE_URL}/" 2>/dev/null || echo '000'
)"

bootstrap_result="$(
  HOME_FILE="${home_body}" HOME_CODE="${home_code}" python3 <<'PY'
import os
import sys

path = os.environ["HOME_FILE"]
code = os.environ["HOME_CODE"]

if code == "000":
    print("FAIL\tno response from /")
    sys.exit(0)

try:
    html = open(path, encoding="utf-8", errors="replace").read().lower()
except OSError as exc:
    print(f"FAIL\tcould not read / body: {exc}")
    sys.exit(0)

markers = [
    "create superuser",
    "create the initial administrator",
    "initial administrator account",
    "show_setup_form",
]
hits = [m for m in markers if m in html]
if hits:
    print(
        "FAIL\t/ looks like open first-run superuser setup — prod should have users already "
        f"(matched: {', '.join(hits[:2])}). Ensure DEBUG=false and database is initialized."
    )
elif "shellui hosting" in html or "shellui deploy" in html or "openapi" in html or "swagger" in html:
    print("PASS\t/ does not expose open superuser signup (hosting landing/docs page)")
elif code in {"301", "302", "303", "307", "308"}:
    print(f"PASS\t/ redirects (HTTP {code}) — bootstrap form not served at apex")
else:
    print("WARN\t/ HTML is ambiguous — could not confirm bootstrap gate; inspect manually")
PY
)" || bootstrap_result=$'WARN\t/ bootstrap check error'
rm -f "${home_body}"

bootstrap_status="${bootstrap_result%%$'\t'*}"
bootstrap_detail="${bootstrap_result#*$'\t'}"
case "${bootstrap_status}" in
  PASS) pass "Bootstrap gate: ${bootstrap_detail}" ;;
  WARN) warn "Bootstrap gate: ${bootstrap_detail}" ;;
  *) fail "Bootstrap gate: ${bootstrap_detail}" ;;
esac

# ---------------------------------------------------------------------------
# 5. Identity / JWKS dependency (informational + health cross-check)
# ---------------------------------------------------------------------------
info "JWT verification requires IDENTITY_JWKS, IDENTITY_JWKS_URL, or IDENTITY_SERVICE_URL → /.well-known/jwks.json."
info "Set IDENTITY_ISSUER / IDENTITY_AUDIENCE when DEBUG=false if tokens use non-default iss/aud."
info "OAuth redirect sync for preview login uses IDENTITY_SERVICE_URL (not CORS) — verify allowlist entries after deploy."

# ---------------------------------------------------------------------------
# 6. Waitlist / access (informational)
# ---------------------------------------------------------------------------
info "Company hosting access waitlist: owner self-approve is staff-only in production — use Django admin or approve_hosting_access."

# ---------------------------------------------------------------------------
# 7. CORS (preview origins for API calls from hosted shells)
# ---------------------------------------------------------------------------
cors_headers="$(mktemp)"
cors_code="$(
  curl -sS -L -D "${cors_headers}" -o /dev/null -w '%{http_code}' \
    -X OPTIONS \
    -H "Origin: ${CORS_PROBE_ORIGIN}" \
    -H 'Access-Control-Request-Method: GET' \
    -H 'Access-Control-Request-Headers: authorization' \
    --connect-timeout 15 --max-time 30 \
    "${BASE_URL}/hosting/v1/health" 2>/dev/null || echo '000'
)"

cors_result="$(
  CORS_HDR_FILE="${cors_headers}" CORS_CODE="${cors_code}" CORS_ORIGIN="${CORS_PROBE_ORIGIN}" python3 <<'PY'
import os
import sys

hdr_path = os.environ["CORS_HDR_FILE"]
code = os.environ["CORS_CODE"]
origin = os.environ["CORS_ORIGIN"]

headers = {}
try:
    with open(hdr_path, encoding="utf-8", errors="replace") as fh:
        for line in fh:
            if ":" not in line:
                continue
            name, value = line.split(":", 1)
            headers[name.strip().lower()] = value.strip()
except OSError as exc:
    print(f"FAIL\tcould not read CORS headers: {exc}")
    sys.exit(0)

acao = headers.get("access-control-allow-origin")
if code == "000":
    print("FAIL\tOPTIONS preflight failed (no response)")
elif acao == "*":
    print(f"PASS\tAccess-Control-Allow-Origin: * (origin {origin})")
elif acao == origin:
    print(f"PASS\tAccess-Control-Allow-Origin reflects {origin}")
elif acao:
    print(
        f"WARN\tAccess-Control-Allow-Origin={acao!r} — random preview origins may be blocked; "
        "product default is allow-all (CORS_ALLOW_ALL_ORIGINS=true)"
    )
else:
    print(
        "FAIL\tno Access-Control-Allow-Origin on OPTIONS — random hosting preview origins will fail in the browser; "
        "set CORS_ALLOW_ALL_ORIGINS=true or add origins to CORS_ALLOWED_ORIGINS"
    )
PY
)"
rm -f "${cors_headers}"

cors_status="${cors_result%%$'\t'*}"
cors_detail="${cors_result#*$'\t'}"
case "${cors_status}" in
  PASS) pass "CORS: ${cors_detail}" ;;
  WARN) warn "CORS: ${cors_detail}" ;;
  *) fail "CORS: ${cors_detail}" ;;
esac

# ---------------------------------------------------------------------------
# 8. Optional public app-host smoke (--slug / --app-host)
# ---------------------------------------------------------------------------
if [[ -n "${APP_HOST_URL}" ]]; then
  app_body="$(mktemp)"
  app_code="$(
    curl -sS -L -o "${app_body}" -w '%{http_code}' \
      --connect-timeout 15 --max-time 30 \
      "${APP_HOST_URL}/" 2>/dev/null || echo '000'
  )"

  app_result="$(
    APP_FILE="${app_body}" APP_CODE="${app_code}" APP_HOST="${APP_HOST_URL}" python3 <<'PY'
import os
import sys

path = os.environ["APP_FILE"]
code = os.environ["APP_CODE"]
host = os.environ["APP_HOST"]

if code == "000":
    print("FAIL\tno response from app host /")
    sys.exit(0)
if code == "500":
    print("FAIL\tHTTP 500 from app host — check ALLOWED_HOSTS, deployment, and storage backend")
    sys.exit(0)

try:
    body = open(path, encoding="utf-8", errors="replace").read()
except OSError as exc:
    print(f"FAIL\tcould not read app host body: {exc}")
    sys.exit(0)

lower = body.lower()
admin_markers = [
    "django administration",
    "log in | django site admin",
    "shellui auth",
    "create the initial administrator",
]
if any(m in lower for m in admin_markers):
    print(
        "FAIL\tapp host returned platform/admin HTML instead of a hosted app — "
        "check DNS points to hosting-service and slug exists"
    )
    sys.exit(0)

if code == "200" and ("<html" in lower or "<!doctype html" in lower):
    print(f"PASS\t{host}/ → HTTP 200 HTML (hosted app or SPA shell)")
elif code == "404" and "site is unavailable" in lower:
    print(f"WARN\t{host}/ → HTTP 404 site unavailable (slug missing/expired — redeploy or pick another slug)")
else:
    print(f"WARN\t{host}/ → HTTP {code} — inspect manually (expected 200 HTML for active preview)")
PY
  )" || app_result=$'WARN\tapp host check error'
  rm -f "${app_body}"

  app_status="${app_result%%$'\t'*}"
  app_detail="${app_result#*$'\t'}"
  case "${app_status}" in
    PASS) pass "App host: ${app_detail}" ;;
    WARN) warn "App host: ${app_detail}" ;;
    *) fail "App host: ${app_detail}" ;;
  esac

  # Path traversal / escape probe on app host
  traverse_paths=(
    '/%2e%2e/%2e%2e/etc/passwd'
    '/assets/../../etc/passwd'
    '/..%2f..%2fetc%2fpasswd'
  )
  for traverse_path in "${traverse_paths[@]}"; do
    traverse_code="$(
      curl -sS -L -o /dev/null -w '%{http_code}' \
        --connect-timeout 15 --max-time 30 \
        "${APP_HOST_URL}${traverse_path}" 2>/dev/null || echo '000'
    )"
    if [[ "${traverse_code}" == "200" ]]; then
      fail "App host traversal: GET ${traverse_path} → HTTP 200 (possible path escape — investigate tar extract / serve rules)"
      break
    elif [[ "${traverse_code}" =~ ^(400|403|404)$ ]]; then
      pass "App host traversal: GET ${traverse_path} → HTTP ${traverse_code} (no escape)"
      break
    elif [[ "${traverse_code}" == "000" ]]; then
      fail "App host traversal: no response for ${traverse_path}"
      break
    fi
  done
  if [[ "${traverse_code:-}" != "200" && ! "${traverse_code:-}" =~ ^(400|403|404)$ && "${traverse_code:-}" != "000" ]]; then
    pass "App host traversal: probe paths returned HTTP ${traverse_code:-?} (not 200 file escape)"
  fi
fi

# ---------------------------------------------------------------------------
# 9. Postgres SSL (Coolify internal DB — informational)
# ---------------------------------------------------------------------------
info "Postgres: hosting-service parses POSTGRES_DATABASE_URL with ssl_require=false (Coolify-internal Postgres often has no TLS)."
info "If you require TLS to an external Postgres, ensure the URL/host supports SSL or adjust database settings in a fork — there is no POSTGRES_SSL_REQUIRE toggle yet."

# ---------------------------------------------------------------------------
# 10. Security headers (warn-only)
# ---------------------------------------------------------------------------
sec_headers="$(mktemp)"
sec_code="$(
  curl -sS -L -D "${sec_headers}" -o /dev/null -w '%{http_code}' \
    --connect-timeout 15 --max-time 30 \
    "${BASE_URL}/" 2>/dev/null || echo '000'
)"

if [[ "${BASE_URL}" == https://* ]]; then
  if grep -qi '^strict-transport-security:' "${sec_headers}" 2>/dev/null; then
    hsts_val="$(grep -i '^strict-transport-security:' "${sec_headers}" | head -1 | cut -d: -f2- | xargs)"
    pass "Security headers: Strict-Transport-Security present (${hsts_val})"
  else
    warn "Security headers: Strict-Transport-Security missing on HTTPS response (configure reverse proxy or Django SECURE_HSTS_*)"
  fi
else
  info "Security headers: skipped HSTS check (BASE_URL is not HTTPS)"
fi
rm -f "${sec_headers}"

# ---------------------------------------------------------------------------
# Summary
# ---------------------------------------------------------------------------
if [[ "${FAILURES}" -gt 0 ]]; then
  printf '\n%d check(s) failed.\n' "${FAILURES}" >&2
  exit 1
fi

printf '\nAll hard checks passed.\n'
exit 0
